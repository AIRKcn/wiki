;(function() {
    ...
    inline._inside = /(?:\[[^\]]*\]|[^\]]|\](?=[^\[]*\]))*/;
    inline._href = /\s*<?(.*?)>?(?:\s+['"]([\s\S]*?)['"])?(?:\s+['"](>{2,}?)['"])?\s*/;
    inline._status = /\s*|X\s*?|x\s*?/g;
    inline.todo = replace(inline.todo)
      ('inside', inline._inside)
      ('status', inline._status)
      ();
    ...
    InlineLexer.prototype.output = function(src) {
      ...
      while (src) {
        ...
        if (cap = this.rules.autolink.exec(src)) {
            // console.log('autolink', src, cap, this);
            src = src.substring(cap[0].length);
            if (cap[2] === '@') {
                text = cap[1][6] === ':'
         ...
         * @Code: Feature Extended
         * @Usage:  [ ] 表示未完成 , [X] 表示已经完成
         */
        if (cap = this.rules.todo.exec(src)) {
            src = src.substring(cap[0].length);
            cap[1] = cap[1].replace(/\s*/g, '').toUpperCase();
            // @FEATURE: 添加了对计划任务说明文字中的二次解析，以便于 `` 代码的显示
            cap[2] = this.output(cap[2]);
            out += ( cap[1] == '' )
                ?
         ...
         * 计划任务列表 todo
         * @Author: Robert <cnwangyl@gmail.com>
         * @Code: Feature Custom Modified
         * @Usage:  [链接文字](连接地址 "title文字" ">>")
         *           链接文字 / 链接地址 / title文字 均与 Markdown 标记要求一致
         *           最后一项，务必使用 " 或 ' 包起来
         *           内容请使用 >> 表示全新窗口打开
         *           内容如果使用 英文或数字标识符 ，则定义为窗口名称，可复用
         *           可理解最后一项为 a 标签中的 target 属性设置
         */
        if (cap = this.rules.link.exec(src)) {
            // console.log('link', src, cap, this);
            src = src.substring(cap[0].length);
    ...
    InlineLexer.prototype.outputLink = function(cap, link) {
        var html = [], icon = '',
            ...
            target = target=='>>' ? '_blank' : escape(target),
            // 过滤title为空的情况
            title = link.title && typeof link.title!=undefined ? link.title.replace(/"/ig, "") : false;
        if (cap[0][0] == '!') {
            // 如果以!开头，按 markdown 表示为图片
            html.push( '<img src="' + escape(link.href) );
            html.push( '" alt="' + escape(cap[1]) + '"' );
            if ( link.title ) {
                html.push( ' title="' + escape(link.title) + '"' );
            }
            html.push( '>' );
        } else if ( cap[0][0]=='?') {
            // 如果以?开头，添加自定义，表示为 词条
            html.push( '<abbr data-ref="' + escape(link.href) + '"' );
            html.push( ' title="' + ( link.title ? escape(link.title) : escape(cap[1]) ) + '">' );
            if ( target == '' ) {
                icon = 'comment';
                html.push( this.output(cap[1]) );
            } else {
                icon = 'new-window';
                html.push( '<a href="'+escape(link.href)+'" target="'+escape(target)+'">' + this.output(cap[1]) + '</a>' );
            }
            html.push( '</abbr> <i class="glyphicon glyphicon-'+icon+'"></i>' );
        } else {
            html.push( '<a href="' + escape(link.href) + '"' );
            html.push( (link.title
             ? ' title="' + escape(link.title) + '"'
             : '') );
            if ( target=='' ) {
                html.push( '>' + this.output(cap[1]) + '</a>' );
            } else {
                html.push( 'target="'+target+'">' + this.output(cap[1]) + '</a> <i class="glyphicon glyphicon-new-window"></i>');
            }
        }
        return html.join('');
    };
...
(function($) {
    ...
    function loadExternalIncludes(parent_dfd) {
        ...
        external_links.each(function (i,e) {
            ...
            var text = $el.toptext();
            // console.log(href);
            // href = ( $.md.isRelativeUrl(href) ) ? $.md.baseUrl + href : href;
            href = $.md.baseUrl + href;
            // console.log(href);
            $.ajax({
                url: href,
                dataType: 'text'
    ...
    // check sub-dir nav
    var inxNavMD = '';
    $.md.InxNavDfd = $.Deferred();
    function registerBuildIndexNav() {
        return false;
        var ajaxReq = {
            url: $.md.baseUrl+'nav.index.md',
            dataType: 'text',
        ...
        try {
            $.ajax(ajaxReq).done(function(data) {
                inxNavMD = data;
                $.md.InxNavDfd.resolve();
            }).fail(function() {
                console.log('no '+ajaxReq.url+' file.');
                $.md.InxNavDfd.reject();
            });
        } catch (e) {
            console.log('e', e);
        ...
        $.md.stage('transform').subscribe(function(done) {
            if (inxNavMD === '') {
                var log = $.md.getLogger();
                log.info('no nav.index.md found, not using a index navbar');
                done();
                return;
            }

            var navHtml = marked(inxNavMD);
            // TODO why are <script> tags from navHtml APPENDED to the jqcol?
            var $h = $('<div style="border:1px solid red;">' + navHtml + '</div>');
            ...
            $('#md-index-list').append($navContent.html());
            done();
        });

        $.md.stage('bootstrap').subscribe(function(done) {
            processPageLinks($('#md-index-list'));
            done();
        });

        $.md.stage('postgimmick').subscribe(function(done) {
            var num_links = $('#md-index-list a').length;
            var has_header = $('#md-index-list .navbar-brand').eq(0).toptext().trim().length > 0;
            if (!has_header && num_links <= 1)
                $('#md-index-list').hide();
...
(function($) {
    ...
    $.md.prepareLink = function(link, options) {
        ...
        if ( link.match(/^(\/\/|http[s]?|file)/g)==null ) {
            // 是本地文件
            return link;
        }
        if (options.forceSSL)
    ...
    function loadRequiredScripts() {
        ...
        $.each(activeLinkTriggers, function(i,trigger) {
            ...
            // console.log('module', activeLinkTriggers, i, trigger, module);
            if (module === undefined) {
...
(function($) {
    ...
    var publicMethods = {
        bootstrapify: function () {
            //console.log('createPageSkeleton');
            createPageSkeleton();
            buildMenu ();
            changeHeading();
    ...
    function createPageSkeleton() {
        ...
        $('#md-index-list').wrap('<div class="container" id="md-index-container"/>');
        $('#md-index-list').wrap('<div class="row" id="md-index-row"/>');
    ...
    function addFooter() {
        ...
        navbar +=     ' <i class="glyphicon glyphicon-copyright-mark"></i> SinaLeju @2015. ';
        navbar +=   '</div>';
        navbar += '</div>';
        var $navbar = $(navbar);
...
// 我的测试
(function($) {
    function index($link, opt, text) {
        // console.log('index()', $link, opt, text);
        if ( $link.length > 0 ) {
        ...
        href = $.md.util.isRelativeUrl(href) ? $.md.baseUrl + href : href;
        ...
        //     console.log(e, $.md, href, title);
        // });
        // $.md.util.isRelativeUrl(href)
        // console.log ( $link, href, title );
        return 'abc';
    }
    var indexGimmick = {
        name: 'index',
        version: $.md.version,
        once: function() {
            //console.log('indexGimmick.once()', this);
            $.md.linkGimmick(this, 'index', index);
        ...
        load: function() {
            // console.log('indexGimmick.load()', this);
            $.md.linkGimmick(this, 'index', index);
            //$.md.stage('')
        }
    };

    $.md.registerGimmick(indexGimmick);
}(jQuery));
...
(function($) {
    ...
    function createAlerts() {
        ...
        matches.each(function() {
            ...
            $p.addClass(box_class);
            var html = $p.find('.md-text').eq(0).html();
            $p.find('.md-text').html('<i class="glyphicon '+icon+'"></i> '+html);
        });
    }
    ...
    function select_paragraphs() {
        var note = ['note', '笔记' ];    // 笔记
        var hint = ['hint', 'tip', 'tips', '提示', '建议' ];    // 建议，提示
        var warning = [ 'attention', 'warning', '警告', '注意' ]; // 警告，注意
        // new add
        var mark = ['mark', '标记', '收藏' ];    // 收藏，标记
        var error = [ 'error', '严重', '错误', '重点', 'question', '问题' ]; // 严重，错误，重点
        var todo = [ 'todo', 'plan', 'done', '待做', '计划', '完成' ]; // 待做 计划 完成

        var exp = note.concat(warning);
        exp = exp.concat(hint);
        exp = exp.concat(mark);
        exp = exp.concat(error);
        exp = exp.concat(todo);
            ...
            $(exp).each (function (i,trigger) {
                ...
                if (txt.match (re) !== null) {
                    if ($.inArray(trigger, note) >= 0) {
                        alertType = 'note';
                    } else if ($.inArray(trigger, mark) >= 0) {
                        alertType = 'mark';
                    } else if ($.inArray(trigger, warning) >= 0) {
                        alertType = 'warning';
                    } else if ($.inArray(trigger, error) >= 0) {
                        alertType = 'error';
                    } else if ($.inArray(trigger, todo) >= 0) {
                        alertType = 'todo';
...
(function($) {
    ...
    var set_theme = function(theme) {
        ...
        $('link[rel=stylesheet][href*="netdna.bootstrapcdn.com"]')
            .remove();
        // $('link[rel=stylesheet][href^="p/c/bootswatch/"]')
        //     .remove();
    ...
    var themechooser = function($links, opt, text) {
        ...
        return $links.each(function(i, e) {
            ...
            $chooser.eq(1).append('<li class="divider" />');
            $chooser.eq(1).append('<li><a href="javascript:;">Current : '+window.localStorage.theme+'</a></li>');
...
// forkmeongithubGimmick
(function($) {
    /* 没啥用，干掉 forkmeongithub @author:王永亮 */
    return false;
    'use strict';
